...
class DataPreprocessor:
    def __init__(self, project_name: str, scaler_type: str, input_params: list, output_params: list, file_name: str):
        ...
        self.param_file = self.project_dir / "params.json"
        ...
        self.file_path = UPLOAD_DIRECTORY / file_name
                ...
                for param in self.output_params:
                    if param == "Throughput":  # Special handling for Throughput
                        ...
                        try:
                            ...
                            valid = False
                            logging.exception(f"Invalid format for 'Throughput' in row {index}")
                            break
                    elif param in row:
    ...
    def clean_data(self, input_df, output_df):
        ...
        logger.info("Cleaned data shape: %s", cleaned_data.shape)
        return cleaned_data
    ...
    def scale_data(self, combined_data):
        ...
        logger.info("Scaling data using %s", self.scaler_type)
    ...
    def save_data(self, X_train, X_test, y_train, y_test):
        ...
        """
        np.savez(
            self.project_dir / "train_data.npz",
            X_train=X_train,
            y_train=y_train,
        )
        np.savez(
            self.project_dir / "test_data.npz",
            X_test=X_test,
            y_test=y_test,
        )
        logger.info("Training and testing data saved.")

    def save_scalers(self, scaler_X, scaler_y):
        """
        Save the scalers to pickle files for future use.
        """
        with (self.project_dir / "scaler_X.pkl").open("wb") as f:
            pickle.dump(scaler_X, f)
        with (self.project_dir / "scaler_y.pkl").open("wb") as f:
    ...
    def preprocess(self):
        ...
        return {
            ...
            "processed_data_preview": cleaned_data.head().to_dict(),
        }
