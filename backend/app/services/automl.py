...
class AutoMLRegressor:
    ...
    def evaluate_model(self, tuner_type, X_test, y_test):
        ...
        best_model = load_model(self.model_dir / f"{self.project_name}_{tuner_type}" / "best_model")
        ...
        if np.any(non_zero_indices):
            mape = (
                np.mean(
                    np.abs(
                        (y_test_inverse[non_zero_indices] - predictions_inverse[non_zero_indices])
                        / y_test_inverse[non_zero_indices]
                    )
                )
                * 100
            )
